...
const ConstTop: React.FC<ConstTopProps> = ({ darkMode, setDarkMode }) => {
  useEffect(() => {
    document.documentElement.style.setProperty(
      '--const-top-bg-color', darkMode ? 'hsl(209, 23%, 22%)' : 'hsl(0, 0%, 100%)'
    );
  }, [darkMode]);

  return (
    <div className={MainStyle.constTop} style={{ backgroundColor: 'var(--const-top-bg-color)', color: 'var(--text-color)' }}>
